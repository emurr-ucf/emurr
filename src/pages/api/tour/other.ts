...
export default async function handler(
  ...
  if (req.method === "POST") {
    ...
    if (
      ...
      return res
        .status(400)
        .json({ error: "Please send a valid userID and tourID." });
        ...
        if (pages) {
          ...
          for (let i = 0; i < pages.length; i++) {
            ...
            if (pageCopy) {
              ...
              fs.copyFile(
                "./websites/" + tour.id + "/" + pages[i].id + ".html",
                "./websites/" + newTour.id + "/" + pageCopy.id + ".html",
                (err) => {
                  if (err) {
              ...
              return res
                .status(400)
                .json({ error: "Page could not be created." });
          }
    ...
    } else return res.status(400).json({ error: "Tour could not be found." });
  }
