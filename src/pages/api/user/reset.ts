...
export default async function handler(
  ...
  if (req.method === "PUT") {
    ...
    if (
      ...
      return res
        .status(400)
        .json({ error: "Please input proper password values." });

    // Error: New password and confirmation password don't match.
    if (newPassword !== confirmPassword)
      return res
        .status(400)
        .json({ error: "New password and confirmation password don't match." });
    ...
    if (user) {
      ...
      if (user.password && comparePass(currentPassword, user.password)) {
        ...
        if (user.password && comparePass(newPassword, user.password))
          return res
            .status(409)
            .json({ error: "New password already in use." });
