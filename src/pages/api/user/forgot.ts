...
export default async function handler(
  ...
  if (req.method === "POST") {
    ...
    if (user) return res.status(200).json({});
      ...
      return res
        .status(409)
        .json({ error: "Could not send forgot password request." });
  }
  ...
  if (req.method === "PUT") {
    ...
    if (!resetPassRequest)
      return res
        .status(404)
        .json({ error: "No reset password request found." });
    ...
    if (user) return res.status(200).json({});
    else return res.status(409).json({ error: "Could not change password." });
  }
}
